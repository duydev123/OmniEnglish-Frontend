...
const SignIn = ({
  ...
  const { Login, GoogleLogin, FacebookLogin } = useUserApi();
    ...
    <div className="w-full max-w-lg mx-auto py-8 px-6 flex flex-col items-center justify-center bg-white">
      ...
      <form onSubmit={HandleSubmit} className="w-full flex flex-col gap-3 border border-slate-200/90 px-8 sm:px-10 py-9 rounded-3xl bg-white shadow-xl shadow-slate-200/50 font-sans">
        ...
        <div className="flex justify-between items-center mt-2">
          <label htmlFor="password" className="text-xs font-bold uppercase tracking-wider text-slate-600">Mật khẩu</label>
          <button type="button" className="text-xs font-bold text-blue-600 hover:text-blue-700 hover:underline cursor-pointer transition-colors" onClick={() => setStage("forgot")}>Quên mật khẩu?</button>
        </div>
        <div className="flex items-center gap-3 border border-slate-200 rounded-xl py-2.5 px-4 bg-slate-50/50 focus-within:bg-white focus-within:border-blue-600 focus-within:ring-3 focus-within:ring-blue-500/15 transition-all duration-200 group">
        ...
        <div className="flex items-center gap-4 my-2">
          ...
          <p className="text-center text-xs font-semibold text-slate-400 uppercase tracking-wider">Hoặc đăng nhập bằng</p>
          <p className="border-t flex-1 border-slate-200"></p>
        </div>
        ...
        <p className="text-center text-xs sm:text-sm text-slate-500 font-medium my-4">
          Chưa có tài khoản?{" "}
          <button className="font-bold text-blue-600 hover:text-blue-700 hover:underline cursor-pointer transition-colors" type="button" onClick={() => setStage("register")}>
            Đăng ký ngay
          </button>
        </p>
      </form>
